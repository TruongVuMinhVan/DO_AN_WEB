...
const FoodSearch = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.error("Lỗi khi load món có sẵn:", err);
      }
    })();
  }, [token]);

  // 4) Đóng dropdown khi click ngoài
  useEffect(() => {
    const onClick = (e) => {
  ...
  // 5) Tính toán tổng dinh dưỡng khi selectedFoods thay đổi
  useEffect(() => {
    const sums = selectedFoods.reduce(
      (acc, f) => ({
  ...
  const handleSearch = async (e) => {
    ...
    try {
      ...
      // Cập nhật lịch sử tìm kiếm (loại bỏ trùng lặp)
      ...
      setHistory((h) => {
        ...
        if (exists) {
          return [{ ...exists, id: Date.now() }, ...h.filter((i) => i.query.toLowerCase() !== q.toLowerCase())];
        }
        return [{ id: Date.now(), query: q }, ...h];
      });
    } catch (error) {
      // Nếu lỗi là 401 (usage limits exceeded) thì cảnh báo cho người dùng
      if (error.response && error.response.status === 401) {
        alert("Nutrition API usage limits exceeded. Please try again later.");
      } else {
  ...
  // 7) Thêm món vào selectedFoods (tăng số lượng nếu đã có)
  const handleAddFood = (food) => {
    ...
    setSelectedFoods((prev) => {
      ...
      if (ex) {
        return prev.map((f) =>
          (f.nix_item_id || f.food_name) === key ? { ...f, quantity: f.quantity + 1 } : f
        );
      }
  ...
  const handleSelectExisting = async (name) => {
    ...
    try {
      ...
      console.error('Error selecting existing food:', error);
    }
  };
    ...
    <div className="food-search-container">
      ...
      <div className="grid-container">
        ...
        <div className="selected-list-panel">
          ...
          <button className="btn-save-meal" onClick={handleSaveMeal}>
            💾 Lưu Bữa Ăn
          </button>
        </div>
